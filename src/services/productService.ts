...
export const findAllProduct = async (request: Request) => {
  ...
  const allProductOnPage: IProduct[] = search
    ? await Product.find(searchFilter).populate('category').skip(skip).limit(limit)
    : await Product.find().populate('category').skip(skip).limit(limit)
  // .sort({price : -1})
